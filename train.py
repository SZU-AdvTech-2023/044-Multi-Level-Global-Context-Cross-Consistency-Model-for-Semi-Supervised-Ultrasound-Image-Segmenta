...
from src.network.boundary_MGCC import Boundary_MGCC
import os
import albumentations as A
import torch.optim.lr_scheduler as lr_scheduler
# from main_model import HiFuse_Small
from src.network.boundary_MGCC_fuse import Boundary_MGCC
...
parser.add_argument('--base_dir', type=str, default="./data/myopia", help='dir')
parser.add_argument('--train_file_dir', type=str, default="syn_train.txt", help='dir')
parser.add_argument('--val_file_dir', type=str, default="rev_OCT_val.txt", help='dir')
...
parser.add_argument('--power', type=float, default=0.9,
                    help='power of adjusting learning rate')
...
def adjust_learning_rate(optimizer, i_iter, args):
    lr = lr_poly(args.base_lr, i_iter, args.max_iterations, args.power)
...
def getModel(args):
    ...
    return Boundary_MGCC(length=args.length, k=args.kernel_size).cuda()
    # return HiFuse_Small().cuda()
...
def train(args):
    ...
    max_epoch = max_iterations // len(trainloader) + 20
    model = getModel(args)
    ...
    for epoch_num in range(max_epoch):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #     param_group['lr'] = lr_
            adjust_learning_rate(optimizer,iter_num,args)
            iter_num = iter_num + 1
